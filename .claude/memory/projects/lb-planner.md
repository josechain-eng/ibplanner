...
- ⚠️ **RIESGO A CONFIRMAR tras deploy:** memoria vieja decía que Cloudflare bloquea fetch directo a bcb.gob.bo (IPs datacenter). Parse verificado localmente (HTTP 200, extrae 11.8), pero falta confirmar que el **Worker** sí alcanza bcb.gob.bo. Verificar con `/dailyinfo?force=1` + `/dailyinfo-log`: si `bcbSource:'bcb'` → resuelto; si cae a `dolarapi` con error `bcb:...` → CF lo bloquea y hay que buscar plan B (proxy u otra fuente real-time). dolarboliviahoy.com sigue JS-rendered (no sirve). Para arreglo inmediato del cache cualquier día: `GET /dailyinfo?force=1`.
