...
- **Cadena de fuentes del oficial (v4.87+, eac918f):** (1) BCB real vía Jina → (2) **dolarbluebolivia.click** directo (Astro server-render, `<span class="faq-official">Bs 11.80</span>`, regex `/faq-official[^>]*>\s*Bs\s*([0-9]{1,2}[.,][0-9]{2})/i`, SIN rate limit) → (3) DolarAPI (13h lag). Cada una corre solo si la anterior falló. `bcbSource` = 'bcb-jina' | 'dolarblue' | 'dolarapi'. dolarboliviahoy.com NO sirve para oficial (solo trae el paralelo en HTML plano; oficial por JS).
